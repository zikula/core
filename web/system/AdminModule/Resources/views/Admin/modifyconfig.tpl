{ajaxheader modname=AdminModule filename=modifyconfig.js noscriptaculous=true effects=true}
{adminheader}
    ...
    <div>
        ...
        <fieldset>
            ...
            <div class="z-formrow">
                ...
                {if $modvars.AdminModuleModule.ignoreinstallercheck eq 1}
        ...
        <fieldset>
            ...
            <div class="z-formrow">
                ...
                {if $modvars.AdminModuleModule.admingraphic eq 1}
            ...
            <div class="z-formrow">
                ...
                <select id="admin_displaynametype" name="modvars[displaynametype]">
                    <option value="1" {if $modvars.AdminModuleModule.displaynametype eq 1}selected="selected"{/if}>{gt text="Display name"}</option>
                    <option value="2" {if $modvars.AdminModuleModule.displaynametype eq 2}selected="selected"{/if}>{gt text="Internal name"}</option>
                    <option value="3" {if $modvars.AdminModuleModule.displaynametype eq 3}selected="selected"{/if}>{gt text="Show both internal name and display name"}</option>
                </select>
            </div>
            <div class="z-formrow">
                <label for="admin_itemsperpage">{gt text="Modules per page in module categories list"}</label>
                <input id="admin_itemsperpage" name="modvars[itemsperpage]" type="text" size="3" maxlength="3" value="{$modvars.AdminModuleModule.itemsperpage|safetext}" />
            </div>
            <div class="z-formrow">
                <label for="admin_modulesperrow">{gt text="Modules per row in admin panel"}</label>
                <input id="admin_modulesperrow" name="modvars[modulesperrow]" type="text" size="3" maxlength="3" value="{$modvars.AdminModuleModule.modulesperrow|safetext}" />
            </div>
            <div class="z-formrow">
                ...
                <select id="admintheme" name="modvars[admintheme]">
                    ...
                    {html_select_themes state='ThemeUtil::STATE_ACTIVE'|const filter='ThemeUtil::FILTER_ADMIN'|const selected=$modvars.AdminModuleModule.admintheme}
                </select>
            </div>
            <div class="z-formrow">
                ...
                <select id="admin_startcategory" name="modvars[startcategory]">
                    ...
                    {if  $modvars.AdminModuleModule.startcategory eq $categories[category].cid}
        ...
        <fieldset>
            ...
            <div class="z-formrow">
                ...
                <select id="admin_defaultcategory" name="modvars[defaultcategory]">
                    {section name=category loop=$categories}
                    {if  $modvars.AdminModuleModule.defaultcategory eq $categories[category].cid}
                    <option value="{$categories[category].cid|safetext}" selected="selected">{$categories[category].name|safetext}</option>
                    {else}
                    <option value="{$categories[category].cid|safetext}">{$categories[category].name|safetext}</option>
                    {/if}
