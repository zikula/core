...
use DateUtil;
use System;
use UsersModule\Controller\FormData\NewUserForm;
use Zikula_Hook_ValidationProviders;
use Zikula_Exception_Fatal;
use UsersModule\Controller\FormData\ModifyUserForm;
use DBUtil;
use LogUtil;
use DateTimeZone;
use DateTime;
use UsersModule\Controller\FormData\ModifyRegistrationForm;
use UsersModule\Controller\FormData\ConfigForm;
use FileUtil;
...
class AdminController extends \Zikula_AbstractController
{
    ...
    public function modifyAction()
    {
        ...
        if ($this->request->getMethod() == 'POST') {
            ...
            if (!$errorFields && !$validators->hasErrors()) {
                if ($originalUser['uname'] != $user['uname']) {
                    ...
                    $updatedUserObj = $this->entityManager->find('UsersModule\Entity\UserEntity', $originalUser['uid']);
        ...
        if ($proceedToForm) {
            ...
            return $this->response($this->view->assign_by_ref('formData', $formData)
                ...
                ->assign('defaultGroupId', ModUtil::getVar('GroupsModule', 'defaultgroup', 1))
                ->assign('primaryAdminGroupId', ModUtil::getVar('GroupsModule', 'primaryadmingroup', 2))
        ...
        if ($this->request->getMethod() == 'POST') {
            ...
            if (!$errorFields && !$validators->hasErrors()) {
                ...
                if ($originalRegistration['uname'] != $registration['uname']) {
                    ...
                    $updatedRegistrationObj = $this->entityManager->find('UsersModule\Entity\UserEntity', $originalRegistration['uid']);
    ...
    public function importAction(array $args = array())
    {
        ...
        $defaultGroup = ModUtil::getVar('GroupsModule', 'defaultgroup');
