...
class RegistrationApi extends \Zikula_AbstractApi
{
    ...
    protected function createUser(array $reginfo, $userNotification = true, $adminNotification = true, $passwordCreatedForUser = '')
    {
        ...
        if (!isset($reginfo['uid']) || empty($reginfo['uid'])) {
            ...
            $user = $this->entityManager->find('UsersModule\Entity\UserEntity', $reginfo['uid']);
        ...
        if ($userObj) {
            ...
            $user = $this->entityManager->find('UsersModule\Entity\UserEntity', $userObj['uid']);
    ...
    public function countAll($args)
    {
        ...
        if (isset($isVerifiedFilter)) {
            ...
            $dql = "SELECT u FROM UsersModule\Entity\UserEntity u $where";
            $query = $this->entityManager->createQuery($dql);
            ...
            $dql = "SELECT COUNT(u.uid) FROM UsersModule\Entity\UserEntity u $where";
            $query = $this->entityManager->createQuery($dql);
    ...
    public function remove($args)
    {
        ...
        if (isset($registration) && $registration) {
            $user = $this->entityManager->find('UsersModule\Entity\UserEntity', $uid);
    ...
    protected function purgeExpired()
    {
        ...
        if ($regExpireDays > 0) {
            ...
            $dql = "
            SELECT v
            FROM UsersModule\Entity\UserVerificationEntity v
            ...
            if (is_array($staleVerifyChgRecs) && !empty($staleVerifyChgRecs)) {
                foreach ($staleVerifyChgRecs as $verifyChg) {
                    ...
                    // delete user record
                    $dql = "DELETE FROM UsersModule\Entity\UserEntity u WHERE u.uid = " . $verifyChg['uid'];
                    $query = $this->entityManager->createQuery($dql);
                    $query->getResult();
    ...
    public function sendVerificationCode($args)
    {
        ...
        $verifyChgObj = new \UsersModule\Entity\UserVerificationEntity;
    ...
    public function getVerificationCode($args)
    {
        ...
        $verifyChg = $this->entityManager->getRepository('UsersModule\Entity\UserVerificationEntity')->findOneby(array('uid' => $args['uid'], 'changetype' => UsersConstant::VERIFYCHGTYPE_REGEMAIL));
        return $verifyChg;
    }
