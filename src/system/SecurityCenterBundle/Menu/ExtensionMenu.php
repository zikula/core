...
namespace Zikula\SecurityCenterBundle\Menu;
...
class ExtensionMenu extends AbstractExtensionMenu
{
    ...
    protected function getAdmin(): ?ItemInterface
    {
        ...
        $menu->addChild('Settings', [
            'route' => 'zikulasecuritycenterbundle_config_config',
        ])->setAttribute('icon', 'fas fa-wrench');
        $menu->addChild('Allowed HTML settings', [
            'route' => 'zikulasecuritycenterbundle_config_allowedhtml',
        ])->setAttribute('icon', 'fas fa-list');

        $outputfilter = $this->variableApi->getSystemVar('outputfilter');
        if (1 === $outputfilter) {
            $menu->addChild('HTMLPurifier settings', [
                'route' => 'zikulasecuritycenterbundle_config_purifierconfig',
            ])->setAttribute('icon', 'fas fa-wrench');
        }
    ...
    {
        return 'ZikulaSecurityCenterBundle';
    }
}
