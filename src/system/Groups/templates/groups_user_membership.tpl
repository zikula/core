...
<form class="z-form" action="{modurl modname="Groups" type="user" func="userupdate" action=$action}" method="post" enctype="application/x-www-form-urlencoded">
    <div>
        ...
        <div class="z-buttons z-formbuttons">
            {button src=button_ok.gif set=icons/extrasmall value="Submit" __alt="Save" __title="Save" __text="Save"}
            <a href="{modurl modname=Groups type=user func=view}" title="{gt text="Cancel"}">{img modname=core src=button_cancel.gif set=icons/extrasmall __alt="Cancel" __title="Cancel"} {gt text="Cancel"}</a>
        </div>
    </div>
</form>
{/if}
