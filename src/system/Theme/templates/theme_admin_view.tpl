        ...
        <tbody>
            ...
            <tr class="{cycle values="z-odd,z-even}{if $theme.name|strtolower eq $currenttheme|strtolower} z-defaulttablerow{/if}">
                ...
                <td class="z-right z-nowrap">
                    {if $theme.admin eq true}
                    <a href="{modurl modname=Admin type=admin func=adminpanel theme=$theme.name}" title="{$theme.displayname|safetext}">{img modname=core src=14_layer_novisible.gif set=icons/extrasmall __alt="Preview" __title="Preview: `$theme.displayname`" class="tooltips"}</a>&nbsp;
                    {else}
                    <a href="{entrypoint}?theme={$theme.name}" title="{$theme.displayname|safetext}">{img modname=core src=14_layer_novisible.gif set=icons/extrasmall __alt="Preview" __title="Preview: `$theme.displayname`" class="tooltips"}</a>&nbsp;
                    {/if}
