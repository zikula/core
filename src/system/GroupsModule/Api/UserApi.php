...
use GroupsModule\Entity\GroupMembershipEntity;
use SecurityUtil;
use DBUtil;
use LogUtil;
use DataUtil;
...
class UserApi extends \Zikula_AbstractApi
{
    ...
    public function getall($args)
    {
        ...
        $qb->select('g')
           ->from('GroupsModule\Entity\GroupEntity', 'g');
    ...
    public function get($args)
    {
        ...
        // get item
        $result = $this->entityManager->find('GroupsModule\Entity\GroupEntity', $args['gid']);
        ...
        $groupmembership = $this->entityManager->getRepository('GroupsModule\Entity\GroupMembershipEntity')->findBy(array('gid' => $args['gid']), array(), $args['numitems'], $args['startnum']);
    ...
    {
        $dql = "SELECT count(g.gid) FROM GroupsModule\Entity\GroupEntity g WHERE g.gtype <> " . CommonHelper::GTYPE_CORE;
    ...
    public function countgroupmembers($args)
    {
        ...
        $dql = "SELECT count(m.gid) FROM GroupsModule\Entity\GroupMembershipEntity m WHERE m.gid = {$args['gid']}";
        $query = $this->entityManager->createQuery($dql);
        return (int)$query->getSingleScalarResult();
    }
    ...
    public function getusergroups($args)
    {
        ...
        $groupmembership = $this->entityManager->getRepository('GroupsModule\Entity\GroupMembershipEntity')->findBy(array('uid' => $args['uid']));
    ...
    public function cancelapp($args)
    {
        ...
        if ($ispending == true) {
            $application = $this->entityManager->getRepository('GroupsModule\Entity\GroupApplicationEntity')->findOneBy(array('gid' => $args['gid'], 'uid' => $args['uid']));
    ...
    public function isuserpending($args)
    {
        ...
        $applications = $this->entityManager->getRepository('GroupsModule\Entity\GroupApplicationEntity')->findBy(array('gid' => $args['gid'], 'uid' => $args['uid']));
    ...
    public function removeuser($args)
    {
        ...
        $membership = $this->entityManager->getRepository('GroupsModule\Entity\GroupMembershipEntity')->findOneBy(array('gid' => $args['gid'], 'uid' => $args['uid']));
    ...
    public function whosonline()
    {
        ...
        $dql = "SELECT DISTINCT(s.uid) FROM UsersModule\Entity\UserSessionEntity s WHERE s.lastused > ' " . $activetime . "' AND s.uid <> 0";
        $query = $this->entityManager->createQuery($dql);
