    ...
    foreach ($rootdirs as $rootdir => $moduletype) {
        if (is_dir($rootdir)) {
            ...
            foreach ($dirs as $dir) {
                ...
                if (file_exists("$rootdir/$dir/pnadmin.php") || is_dir("$rootdir/$dir/pnadmin") || file_exists("$rootdir/$dir/Admin.php")) {
                    $adminCapable = ZYES;
                    $modtype = $moduletype;
                } else {
                    $adminCapable = ZNO;
                ...
                if (file_exists("$rootdir/$dir/pnuser.php") || is_dir("$rootdir/$dir/pnuser") || file_exists("$rootdir/$dir/User.php")) {
                    $userCapable = ZYES;
                    if (!isset($modtype)) {
                        $modtype = $moduletype;
                    }
                ...
                if (is_dir("modules/$dir/locale")) {
                    ...
                    // this results in __() caching the result before the domain is bounded.  Will not occur in zOO
...
function modules_adminapi_getlinks()
{
    ...
    if (SecurityUtil::checkPermission('Modules::', '::', ACCESS_ADMIN)) {
        $links[] = array('url' => ModUtil::url('Modules', 'admin', 'view'), 'text' => __('Modules list'), 'class' => 'z-icon-es-list');
    }
    if (SecurityUtil::checkPermission('Modules::', '::', ACCESS_ADMIN)) {
        $links[] = array('url' => ModUtil::url('Modules', 'admin', 'hooks', array('id' => 0)), 'text' => __('System hooks'), 'class' => 'z-icon-es-package');
        $links[] = array('url' => ModUtil::url('Modules', 'admin', 'modifyconfig'), 'text' => __('Settings'), 'class' => 'z-icon-es-config');
    }
