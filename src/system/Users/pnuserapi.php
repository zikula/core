 ...
 */
function users_userapi_mailconfirmationcode($args)
{
    $emailMessageSent = false;
    ...
    if (!$user) {
        ...
        if ($confirmationCodeHash !== false) {
            ...
            if ($codeSaved) {
                ...
                if (!$emailMessageSent) {
                    LogUtil::registerError(__('Error! Unable to send confirmation code e-mail message.'));
                }
            } else {
                LogUtil::registerError(__('Error! Unable to save confirmation code.'));
            }
        } else {
            LogUtil::registerError(__("Error! Unable to create confirmation code."));
        }
    }
...
function Users_userapi_accountlinks()
{
    ...
    foreach ($mods as $mod) {
        ...
        $oopath = DataUtil::formatForOS("$modpath/{$mod['directory']}/lib/Api/Account.php");
        if (file_exists($oopath) || file_exists("$modpath/".DataUtil::formatForOS($mod['directory']).'/pnaccountapi.php')) {
