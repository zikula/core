...
<div class="z-admincontainer">
    ...
    <div class="z-buttons z-center" style="margin:0 0 1em; padding:1em 0;">
        ...
        {/if}
        <a href="{$regactions.verify|safetext}">{img src='mail_send.gif' modname='core' set='icons/extrasmall' title=$actionTitle alt=$actionTitle}{$actionTitle}</a>
        {/if}

        {if isset($regactions.approveForce) && $regactions.approveForce && !$reginfo.isverified}
        <a href="{$regactions.approveForce|safetext}">{img src='db_update.gif' modname='core' set='icons/extrasmall' __title='Skip verification (approves, and creates a new user account)' __alt='Skip verification (approves, and creates a new user account)'} {gt text='Add user without verification'}</a>
        {/if}

        <a href="{modurl modname='Users' type='admin' func='viewRegistrations' restoreview='true'}">{img modname='core' src='button_cancel.gif' set='icons/extrasmall'  __alt="Return to registrations list" __title="Return to registrations list"} {gt text='Return to registrations'}</a>
        {/strip}
    </div>
</div>
