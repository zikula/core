...
class Blocks_Block_Menutree extends Zikula_Block
{
    ...
    public function display($blockinfo)
    {
        ...
        if (!isset($vars['menutree_tpl']) || empty($vars['menutree_tpl']) || !$this->view->template_exists($vars['menutree_tpl'])) {
            $vars['menutree_tpl'] = 'blocks_block_menutree.tpl';
        }
                ...
                if($perms || in_array($item['parent'], $blocked)) {
                    ...
                    if(strpos($item['href'],'{menutree:ext:') === 0) {
                        ...
                        if(!empty($dynamic[1]) && !empty($dynamic[2])) {
                            ...
                            $node = ModUtil::apiFunc('menutree','external',$dynamic[2],$args);
                            if(!is_array($node)) {
    ...
    public function modify($blockinfo)
    {
        ...
        return $this->view->fetch('blocks_block_menutree_modify.tpl');
    }
    ...
    public function update($blockinfo)
    {
        ...
        if($menutree_menus != 'null') {
            ...
            // json_decode needs data in utf so convert it
            $vars['menutree_content'] = DataUtil::convertToUTF8($vars['menutree_content']);
            if (function_exists('json_decode')) {
                ...
                $vars['menutree_content'] = $json->decode($vars['menutree_content']);
            }
            $vars['menutree_content'] = DataUtil::convertFromUTF8($vars['menutree_content']);
        }
