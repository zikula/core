...
class BlocksModuleInstaller extends \Zikula_AbstractInstaller
{
    ...
    protected function migrateMessages()
    {
        ...
        if ($data) {
            foreach ($data as $key => $value) {
                ...
                $data[$key]['mid'] = ModUtil::getIdFromName('ZikulaBlocksModule');
    ...
    protected function newBlockPositions()
    {
        ...
        if (!isset($positions['search'])) {
            $searchpid = ModUtil::apiFunc('ZikulaBlocksModule', 'admin', 'createposition', array('name' => 'search', 'description' => $this->__('Search block')));
        } else {
        ...
        if (empty($searchblocks)) {
            ...
            $block['bid'] = ModUtil::apiFunc('ZikulaBlocksModule', 'admin', 'create', $block);
        ...
        if (!isset($positions['topnav'])) {
            ...
            ZLanguage::setLocale($saveLanguage);
            $topnavcontent = serialize($topnavcontent);
            $topnavblock = array('bkey' => 'Extmenu', 'collapsable' => 1, 'defaultstate' => 1, 'language' => '', 'mid' => ModUtil::getIdFromName('ZikulaBlocksModule'), 'title' => $this->__('Top navigation'), 'description' => '', 'content' => $topnavcontent, 'positions' => array($topnav));
            $topnavblock['bid'] = ModUtil::apiFunc('ZikulaBlocksModule', 'admin', 'create', $topnavblock);
            ModUtil::apiFunc('ZikulaBlocksModule', 'admin', 'update', $topnavblock);
        }
    }
}