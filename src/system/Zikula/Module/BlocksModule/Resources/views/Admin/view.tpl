    ...
    <fieldset>
        ...
        <span class="nowrap">
            ...
            <select id="filter_status" name="filter[active_status]">
                <option value="0"{if (isset($filter.active_status) && $filter.active_status eq 0)} selected="selected"{/if}>{gt text='All'}</option>
                <option value="1"{if (isset($filter.active_status) && $filter.active_status eq 1)} selected="selected"{/if}>{gt text='Active'}</option>
                <option value="2"{if (isset($filter.active_status) && $filter.active_status eq 2)} selected="selected"{/if}>{gt text='Inactive'}</option>
            </select>
        </span>
        <span class="nowrap">
...
<table class="table table-bordered table-striped">
    <thead>
        <tr>
            <th>{sortlink __linktext='Block ID' sort='bid' currentsort=$sort sortdir=$sortdir route='zikulablocksmodule_admin_view' filter=$filter}</th>
            <th>{sortlink __linktext='Title' sort='title' currentsort=$sort sortdir=$sortdir route='zikulablocksmodule_admin_view' filter=$filter}</th>
            <th>{sortlink __linktext='Description' sort='description' currentsort=$sort sortdir=$sortdir route='zikulablocksmodule_admin_view' filter=$filter}</th>
            <th>{gt text='Module'}</th>
            <th>{sortlink __linktext='Name' sort='bkey' currentsort=$sort sortdir=$sortdir route='zikulablocksmodule_admin_view' filter=$filter}</th>
            <th>{gt text='Position(s)'}</th>
            <th>{sortlink __linktext='Language' sort='language' currentsort=$sort sortdir=$sortdir route='zikulablocksmodule_admin_view' filter=$filter}</th>
            <th>{sortlink __linktext='State' sort='active' currentsort=$sort sortdir=$sortdir route='zikulablocksmodule_admin_view' filter=$filter}</th>
