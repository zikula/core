...
class AdminController extends \Zikula_AbstractController
{
     ...
     *
     * @throws AccessDeniedHttpException if the user doesn't have edit permission to any groups
     *
     * @return void
     */
     ...
     *
     * @throws AccessDeniedHttpException Thrown if the user hasn't permissions to administer any groups
     */
     ...
     * @return void
     *
     * @throws \InvalidArgumentException Thrown if a group with the requested name already exists
     */
    public function createAction(array $args = array())
    {
        ...
        if ($check != false) {
            ...
            throw new \InvalidArgumentException($this->__('Error! There is already a group with that name.'));
        } else {
    ...
    public function deleteAction(array $args = array())
    {
        ...
        if ($item['gid'] == $defaultgroup) {
            throw new \InvalidArgumentException($this->__('Error! You cannot delete the default user group.'));
        }
     ...
     * @return void
     *
     * @throws \RuntimeException Throw if any users couldn't be added to the requested group
     */
    public function adduserAction(array $args = array())
    {
        ...
        if (is_array($uid)) {
            ...
            if ($total_users_notadded > 0) {
                throw new \RuntimeException($this->_fn('Error! %s user was not added to the group.', 'Error! %s users were not added to the group.', $total_users_notadded, $total_users_notadded));
            }
        } else {
            if (!ModUtil::apiFunc('ZikulaGroupsModule', 'admin', 'adduser', array('gid' => $gid, 'uid' => $uid))) {
                throw new \RuntimeException($this->__('Error! A problem occurred and the user was not added to the group.'));
            } else {
    ...
    public function removeuserAction(array $args = array())
    {
        ...
        if (ModUtil::apiFunc('ZikulaGroupsModule', 'admin', 'removeuser', array('gid' => $gid, 'uid' => $uid))) {
            ...
            throw new \RuntimeException($this->__('Error! A problem occurred while attempting to remove the user. The user has not been removed from the group.'));
        }
    ...
    public function userupdateAction()
    {
        ...
        if (!$result) {
            if ($action == 'deny') {
                throw new \RuntimeException($this->__("Error! Could not execute 'Reject' action."));
            } else {
                throw new \RuntimeException($this->__("Error! Could not execute 'Accept' action."));
            }
            return $this->redirect(ModUtil::url('ZikulaGroupsModule', 'admin', 'view'));
        }
