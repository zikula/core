...
<div class="z-warningmsg">
    ...
    {if $authentication_method.modname != 'UsersModule'}<p>{gt text="Note: This changes the password for your account with the user name of '%1$s', here on this web site. It does not affect the password for any other method of logging in, such as the method you just used."  tag1=$user_obj.uname}</p>{/if}
...
<form id="users_user_changepassword" class="z-form" action="{modurl modname="Users" type="user" func="updatePassword"}" method="post">
    <div>
        ...
        <div class="z-formbuttons z-buttons">
            ...
            {/if}
            <a href="{modurl modname='UsersModule' type='user' func='index'}" title="{gt text='Cancel'}" class='z-bt-cancel'>{gt text='Cancel'}</a>
        </div>
    </div>
</form>
