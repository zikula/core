    ...
    <tbody>
        ...
        {foreach from=$reglist item='reginfo' name='reglist'}<tr class="{cycle values='z-odd,z-even'}">
            ...
            <td class="z-center">
                ...
                {if ($modvars.Users.moderation_order != 'UsersModule\Constant::APPROVAL_BEFORE'|const) || (($modvars.Users.moderation_order == 'UsersModule\Constant::APPROVAL_BEFORE'|const) && ($reginfo.isapproved))}
            ...
            <td class="users_action">
                ...
                {if isset($modvars.Users.moderation_order) && ($modvars.Users.moderation_order == 'UsersModule\Constant::APPROVAL_AFTER'|const)}
...
<table class="z-datatable">
    ...
    <tbody>
        ...
        <tr>
            <td class="z-w45">
                <div title="{gt text='Verification e-mail message not yet sent'}" class="z-sub user-icon-mail_delete">{gt text='A verification e-mail has not been sent to the registered e-mail address.'}{if $modvars.Users.moderation_order == 'UsersModule\Constant::APPROVAL_BEFORE'|const} {gt text='If it is not yet approved, then it will be sent on approval.'}{/if}</div>
            </td>
            <td class="z-w45">
