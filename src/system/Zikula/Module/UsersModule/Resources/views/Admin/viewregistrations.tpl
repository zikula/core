...
<table class="table table-bordered">
    ...
    <tbody>
        <tr>
            <td class="z-w45">
                <div title="{gt text='Approved'}" class="sub user-icon-greenled">{gt text='An administrator has approved the registration, or approval was not required when the registration was completed.'}</div>
            </td>
            <td class="z-w45">
                <div title="{gt text='Pending approval'}" class="sub user-icon-redled">{gt text='Waiting for an administrator to approve the registration.'}</div>
            </td>
        </tr>
    </tbody>
</table>
<table class="table table-bordered">
    ...
    <tbody>
        <tr>
            <td class="z-w45">
                <div title="{gt text='Verified'}" class="sub user-icon-greenled">{gt text='The user has completed the e-mail verification process, or e-mail verification was not required when the registration was completed.'}</div>
            </td>
            <td class="z-w45">
                <div title="{gt text='Pending verification of e-mail address'}" class="sub user-icon-redled">{gt text='An e-mail has been sent to the registered e-mail address, but the user has not yet responded.'}</div>
            </td>
        </tr>
        <tr>
            <td class="z-w45">
                <div title="{gt text='Verification e-mail message not yet sent'}" class="sub user-icon-mail_delete">{gt text='A verification e-mail has not been sent to the registered e-mail address.'}{if $modvars.ZikulaUsersModule.moderation_order == 'Zikula\Module\UsersModule\Constant::APPROVAL_BEFORE'|const} {gt text='If it is not yet approved, then it will be sent on approval.'}{/if}</div>
            </td>
            <td class="z-w45">
                <div title="{gt text='E-mail verification not sent; must be resent'}" class="sub user-icon-status_unknown">{gt text='A verification e-mail has not been sent to the registered e-mail address, but the registration is in a state where one should already have been sent.'}</div>
            </td>
        </tr>
    </tbody>
</table>
