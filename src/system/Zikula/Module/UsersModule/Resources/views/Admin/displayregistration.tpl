...
<div class="text-center">
    ...
    <a class="btn btn-success" href="{$regactions.approve|safetext}"><span class="icon-ok"></span> {gt text='Approve'}</a>&nbsp;
    {elseif isset($regactions.approve) && $regactions.approve && $reginfo.isverified}
    <a class="btn btn-success" href="{$regactions.approve|safetext}"><span class="icon-plus"></span> {gt text='Approve and Add User'}</a>&nbsp;
    {/if}
    {if isset($regactions.deny) && $regactions.deny}
    <a class="btn btn-danger" href="{$regactions.deny|safetext}"><span class="icon-trash"></span> {gt text='Deny'}</a>&nbsp;
    {/if}
    {if isset($regactions.modify) && $regactions.modify}
    <a class="btn btn-warning" href="{$regactions.modify|safetext}"><span class="icon-pencil"></span> {gt text='Edit'}</a>&nbsp;
    {/if}
    ...
    {/if}
    <a class="btn btn-info" href="{$regactions.verify|safetext}"><span class="icon-envelope"></span> {$actionTitle}</a>&nbsp;
    {/if}

    {if isset($regactions.approveForce) && $regactions.approveForce && !$reginfo.isverified}
        <a class="btn btn-success" href="{$regactions.approveForce|safetext}" title="title='Skip verification (approves, and creates a new user account)'}"><span class="icon-plus"></span> {gt text='Add user without verification'}</a>&nbsp;
    {/if}

    <a class="btn btn-default" href="{modurl modname='ZikulaUsersModule' type='admin' func='viewRegistrations' restoreview='true'}"><span class="icon-reply"></span> {gt text='Return to registrations'}</a>&nbsp;
    {/strip}
</div>
