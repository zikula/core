...
class RegistrationApi extends \Zikula_AbstractApi
{
    ...
    public function countAll($args)
    {
        ...
        if (isset($isVerifiedFilter)) {
            ...
            $dql = "SELECT u FROM Zikula\Module\UsersModule\Entity\UserEntity u $where";
            $query = $this->entityManager->createQuery($dql);
            ...
            $dql = "SELECT COUNT(u.uid) FROM Zikula\Module\UsersModule\Entity\UserEntity u $where";
            $query = $this->entityManager->createQuery($dql);
        ...
        if ($regExpireDays > 0) {
            ...
            if (is_array($staleVerifyChgRecs) && !empty($staleVerifyChgRecs)) {
                foreach ($staleVerifyChgRecs as $verifyChg) {
                    ...
                    // delete user record
                    $dql = "DELETE FROM Zikula\Module\UsersModule\Entity\UserEntity u WHERE u.uid = " . $verifyChg['uid'];
                    $query = $this->entityManager->createQuery($dql);
                    $query->getResult();
