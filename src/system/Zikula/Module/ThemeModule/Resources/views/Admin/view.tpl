    ...
    <tbody>
        ...
        <tr {if $theme.displayname|strtolower eq $currenttheme|strtolower}class="success"{/if}>
            ...
            <td class="actions">
                ...
                <a href="{modurl modname="ZikulaThemeModule" type="admin" func="setasdefault" themename=$theme.displayname}"><span class="icon-ok tooltips" title="{$strSetDefaultTheme}"></span></a>
                {/if}
                {if $theme.structure}
                <a href="{$themeurl|safetext}" title="{$theme.displayname|safetext}"><span class="icon-eye-open tooltips" title="{$strPreviewTheme}"></span></a>
                <a href="{modurl modname="ZikulaThemeModule" type="admin" func="modify" themename=$theme.displayname}"><span class="icon-pencil tooltips" title="{$strEditTheme}"></span></a>
                {/if}
                {if $theme.name neq $currenttheme and $theme.state neq 2}
                <a href="{modurl modname="ZikulaThemeModule" type="admin" func="delete" themename=$theme.displayname}"><span class="icon-trash tooltips" title="{$strDeleteTheme}"></span></a>
                {/if}
                <a href="{modurl modname="ZikulaThemeModule" type="admin" func="credits" themename=$theme.displayname}"><span class="icon-info-sign tooltips" title="{$strCreditsTheme}"></span></a>
            </td>
        </tr>
        {foreachelse}
