...
<form id="themeform" action="" method="post" enctype="application/x-www-form-urlencoded">
    <div>
        ...
        <select id="newtheme" name="newtheme">
            {foreach from=$themes item=theme}
            <option id="theme_{$theme.directory}" title="{$theme.description}" value="{$theme.directory}"{if $theme.name eq $currenttheme.name} selected="selected"{/if} data-previewimage="{$baseurl}{$theme.previewImage}">{$theme.displayname}</option>
            {/foreach}
        </select>
    </div>
