...
class Search_User extends Zikula_Controller
{
    ...
    public function form($vars = array())
    {
        ...
        if (count($search_modules) > 0) {
            ...
            $renderer->assign('plugin_options', $plugin_options);

            // Return the output that has been generated by this function
            return $renderer->fetch('search_user_form.htm');
        } else {
            // Create output object
            // Return the output that has been generated by this function
            return $renderer->fetch('search_user_noplugins.htm');
        }
    }
    ...
    public function search()
    {
        ...
        $renderer = Renderer::getInstance('Search');
        $renderer->cache_id = md5($vars['q'] . $vars['searchtype'] . $vars['searchorder'] . UserUtil::getVar('uid')) . $vars['page'];
        // check if the contents are cached.
        if ($renderer->is_cached('search_user_results.htm')) {
            return $renderer->fetch('search_user_results.htm');
        }
        ...
        $renderer->assign('resultcount', $result['resultCount']);
        $renderer->assign('results', $result['sqlResult']);
        $renderer->assign(ModUtil::getVar('Search'));
        $renderer->assign($vars);
        $renderer->assign('limitsummary', $limitsummary);
    ...
    public function recent()
    {
        ...
        $renderer->assign('recentsearches', $items);

        // assign the values for the smarty plugin to produce a pager in case of there
        // being many items to display.
        $renderer->assign('pager', array('numitems'     => ModUtil::apiFunc('Search', 'user', 'countitems'),
                'itemsperpage' => $itemsperpage));

        // Return the output that has been generated by this function
        return $renderer->fetch('search_user_recent.htm');
    }
}