...
/**
 * FileSystem_SFtp is the standard driver for SFTP connections. This class extends FileSystem_Driver
...
class FileSystem_SFtp extends FileSystem_Driver
{
    ...
    private $dir;
    /**
     * Standard function for creating a SFTP connection and logging in, this must be called
    ...
    {
        $this->start_handler();
        if (($this->ssh_resource = ssh2_connect($this->configuration->getHost(),
            $this->configuration->getPort())) !== FALSE) {
            //connected
            if ((ssh2_auth_password($this->ssh_resource, $this->configuration->
                getUser(), $this->configuration->getPass())) !== FALSE) {
                //logged in
                if (($this->resource = ssh2_sftp($this->ssh_resource)) !== FALSE
                    ) {
                    //started sftp
                    if (($this->dir = ssh2_sftp_realpath($this->resource, $this
                        ->configuration->getDir())) !== FALSE) {
                        //changed dir
                        $this->stop_handler();
    ...
    {
        $remote = ($remote == "" || substr($remote, 0, 1) !== "/" ? $this->dir.
            '/'.$remote : $remote);
        $res = $this->resource;
        $this->start_handler();
        if (($bytes = file_put_contents("ssh2.sftp://$this->resource/$remote",
            $stream, 0)) !== FALSE) {
            fclose($stream);
            $this->stop_handler();
            return $bytes;
    ...
    public function fget($remote)
    {
        $remote = ($remote == "" || substr($remote, 0, 1) !== "/" ? $this->dir.
            '/'.$remote : $remote);
        $this->start_handler();
        if (($handle = fopen("ssh2.sftp://$this->resource/$remote", 'r+')) !==
            FALSE) {
            rewind($handle);
            $this->stop_handler();
            return $handle;
    ...
    {
        $this->start_handler();
        $file = ($file == "" || substr($file, 0, 1) !== "/" ? $this->dir.'/'.
            $file : $file);
        ...
        $this->error_handler('0',
            "Did not get acknowledgment from host, chmod may or may not have succeeded.",
            '', '');
        return false;
    }
    ...
    {
        $dir = ($dir == "" || substr($dir, 0, 1) !== "/" ? $this->dir.'/'.$dir :
            $dir);
        ...
        $this->error_handler(0, "Directory: $dir is not a directory or does not exist",
            'SFtp.php', '0');
        return false;
    }
    ...
    {
        $this->start_handler();
        $sourcepath = ($sourcepath == "" || substr($sourcepath, 0, 1) !== "/" ?
            $this->dir.'/'.$sourcepath : $sourcepath);
        $destpath = ($destpath == "" || substr($destpath, 0, 1) !== "/" ? $this
            ->dir.'/'.$destpath : $destpath);
        if (($sourcepath = ssh2_sftp_realpath($this->resource, $sourcepath)) !==
            FALSE) {
            if ((ssh2_sftp_rename($this->resource, $sourcepath, $destpath)) !==
                FALSE) {
                //renamed file
                $this->stop_handler();
                return true;
    ...
    public function cp($sourcepath, $destpath)
    {
        ...
        $this->stop_handler();
        $this->error_handler('0',
            "Did not get acknowledgment from host, cp may or may not have succeeded.",
            '', '');
        return false;
    }
    ...
    private function error_codes()
    {
        ...
        $errors = array(
            ...
            array('code' => '9', 'search' => 'Permission denied'),
            array('code' => '9', 'search' => 'Authentication failed'),
            array('code' => '10', 'search' => 'Operation not permitted'), //usually means your host has chmod disabled
            array('code' => '11', 'search' => 'Delete operation failed'),
            array('code' => '12', 'search' => 'not a valid resource handle'), //TODO we should never get this, fix it so we dont!
            array('code'   => '13', 'search' => 'Unable to resolve realpath for'
            )
        );
        return $errors;
    }
