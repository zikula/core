...
class ModUtil
{
    ...
    public static function dbInfoLoad($modname, $directory = '', $force = false)
    {
        ...
        $file = "$modpath/$directory/tables.php";
    ...
    public static function exec($modname, $type = 'user', $func = 'main', $args = array(), $api = false, $instanceof = null)
    {
        ...
        if ($loaded) {
            ...
            if (file_exists($file = "config/functions/$modname/{$type}{$ftype}/$func.php")) {
                include_once $file;
                if (is_callable($modfunc)) {
                    $eventManager->notify($preExecuteEvent);
                    $postExecuteEvent->setData($modfunc($args));
     ...
     */
    public static function url($modname, $type, $func, $args = array(), $ssl = null, $fragment = null, $fqurl = null, $forcelongurl = false, $forcelang=false)
    {
        // define input, all numbers and booleans to strings
        $modname = isset($modname) ? ((string)$modname) : '';
