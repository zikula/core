...
class UserUtil
{
     ...
     */
    public static function getGroups($where = [], $orderBy = [], $limitOffset = null, $limitNumRows = null, $assocKey = 'gid')
    {
        $em = \ServiceUtil::get('doctrine.entitymanager');
    ...
    public static function getSelectorData_Group($defaultValue = 0, $defaultText = '', $ignore = [], $includeAll = 0, $allText = '')
    {
        ...
        if ($defaultText) {
            $dropdown[] = ['id' => $defaultValue, 'name' => $defaultText];
        }
        ...
        if ($includeAll) {
            $dropdown[] = ['id' => $includeAll, 'name' => $allText];
        }

        foreach (array_keys($groupdata) as $gid) {
            if (!isset($ignore[$gid])) {
                $gname = $groupdata[$gid]['name'];
                $dropdown[$gname] = ['id' => $gid, 'name' => $gname];
            }
        }
     ...
     * @return array The array structure for the user group selector.
     */
    public static function getSelectorData_User($defaultValue = 0, $defaultText = '', $ignore = [], $includeAll = 0, $allText = '', $exclude = '')
    {
        $dropdown = [];
        ...
        foreach (array_keys($userdata) as $uid) {
            if (!isset($ignore[$uid])) {
                ...
                $dropdown[$uname] = ['id' => $uid, 'name' => $uname];
            }
        }
