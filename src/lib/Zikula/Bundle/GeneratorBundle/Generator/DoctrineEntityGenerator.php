...
class DoctrineEntityGenerator extends Generator
{
    ...
    public function generate(BundleInterface $module, $entity, $format, array $fields, $withRepository)
    {
        // configure the module (needed if the module does not contain any Entities yet)
        $config = $this->registry->getEntityManager(null)->getConfiguration();
        $config->setEntityNamespaces(array_merge(
            array($module->getName() => $module->getNamespace().'\\Entity'),
            $config->getEntityNamespaces()
        ));

        $entityClass = $this->registry->getEntityNamespace($module->getName()).'\\'.$entity;
        $entityPath = $module->getPath().'/Entity/'.str_replace('\\', '/', $entity).'.php';
        ...
        if ('annotation' === $format) {
            ...
            $mappingPath = $module->getPath().'/Resources/config/doctrine/'.str_replace('\\', '.', $entity).'.orm.'.$format;
        ...
        if ($withRepository) {
            $path = $module->getPath().str_repeat('/..', substr_count(get_class($module), '\\'));
