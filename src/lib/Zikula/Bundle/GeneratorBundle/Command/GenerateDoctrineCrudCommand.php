...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $generator->generate($module, $entity, $metadata[0], $format, $prefix, $withWrite, $forceOverwrite);
        ...
        // form
        if ($withWrite) {
            $this->generateForm($module, $entity, $metadata);
            $output->writeln('Generating the Form code: <info>OK</info>');
        }

        // routing
        if ('annotation' != $format) {
            $runner($this->updateRouting($dialog, $input, $output, $module, $format, $entity, $prefix));
        }
    ...
    protected function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln(array(
            ...
            '',
            sprintf("You are going to generate a CRUD controller for \"<info>%s:%s</info>\"", $module, $entity),
     ...
     */
    protected function generateForm($module, $entity, $metadata)
    {
        try {
            $this->getFormGenerator()->generate($module, $entity, $metadata[0]);
        } catch (\RuntimeException $e ) {
            // form already exists
        }
    }

    protected function updateRouting($dialog, InputInterface $input, OutputInterface $output, $module, $format, $entity, $prefix)
    {
        $auto = true;
        if ($input->isInteractive()) {
        ...
        $this->getContainer()->get('filesystem')->mkdir($module->getPath().'/Resources/config/');
        $routing = new RoutingManipulator($module->getPath().'/Resources/config/routing.yml');
        try {
            $ret = $auto ? $routing->addResource($module->getName(), $format, '/'.$prefix, 'routing/'.strtolower(str_replace('\\', '_', $entity))) : false;
        } catch (\RuntimeException $exc) {
            $ret = false;
        }

        if (!$ret) {
            $help = sprintf("        <comment>resource: \"@%s/Resources/config/routing/%s.%s\"</comment>\n", $module->getName(), strtolower(str_replace('\\', '_', $entity)), $format);
            $help .= sprintf("        <comment>prefix:   /%s</comment>\n", $prefix);

            return array(
                '- Import the module\'s routing resource in the module routing file',
                sprintf('  (%s).', $module->getPath().'/Resources/config/routing.yml'),
                '',
                sprintf('    <comment>%s:</comment>', $module->getName().('' !== $prefix ? '_'.str_replace('/', '_', $prefix) : '')),
                $help,
                '',
            );
        }
